...
Item {
  ...
  function stopFace(message) {
    ...
    faceTimeout.stop()
    if (facePam.active) facePam.abort()
    if (message && lockRequested) faceMessage = String(message)
    if (wasAuthenticating) logEvent("face-cancelled")
  }
  ...
  function startFace() {
    ...
    if (!facePam.start()) {
      stopFace("Face scan unavailable. You can use your password.")
      return
    }
    faceTimeout.restart()
  ...
  function handleFaceFinished(result) {
    // Ignore late completions after cancellation or an unlock by another method.
    var wasAuthenticating = faceAuthenticating
    faceAuthenticating = false
    faceTimeout.stop()
    if (!wasAuthenticating || !lockRequested || !sessionLock.secure) return
    logEvent(result === PamResult.Success ? "face-success" : "face-rejected")
    if (result === PamResult.Success) finishUnlock()
    else faceMessage = "Face check did not pass. Turn slightly and press Enter to retry, or use your password."
  }
  ...
  WlSessionLock {
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        onCancelFaceRequested: root.stopFace("Face scan cancelled. Press Enter to retry, or use your password.")
  ...
  PamContext {
    ...
    onError: function(error) {
      if (root.faceAuthenticating) root.stopFace("Face scan unavailable. You can use your password.")
    }
    // This lane must never request or receive the user's password.
    onResponseRequiredChanged: {
      if (responseRequired && root.faceAuthenticating) root.stopFace("Face scan unavailable. You can use your password.")
    }
  }
  ...
  onFaceAuthenticatingChanged: {
    if (lockRequested && !faceAuthenticating) armBlankTimer()
  }
